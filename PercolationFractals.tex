...
We will write $P \sim \text{Perc}(n,p,\infty)$ for $P = \bigcap_{k \to \infty} P^k$, with $P^k$ defined recursively as above.
...
All plots from our simulations are available online\footnote{See \url{https://pauldubois98.github.io/PercolationFractalsStudy/blob.html}.}.
...
%$$\underbrace{(1-p)n^D}_{\substack{\text{Expected number}\\ \text{of cuboids removed}}} * \underbrace{2D \left( \nicefrac{1}{n} \right)^{D-1}}_{\substack{\text{Boundary of a single}\\ \text{cuboid of side } \nicefrac{1}{n}}} + \underbrace{2D}_{\substack{\text{Boundary of the}\\ \text{unit cuboid}}} = 2D (1 + (1-p)n) \to \infty \quad \text{ as } n \to \infty$$
$$\underbrace{(1-p)n^D}_{\substack{\text{Expected number}\\ \text{of cuboids removed}}} \overbrace{2D \left( \nicefrac{1}{n} \right)^{D-1}}^{\substack{\text{Boundary of a single}\\ \text{cuboid of side } \nicefrac{1}{n}}} + \underbrace{2D}_{\substack{\text{Boundary of the}\\ \text{unit cuboid}}} = 2D (1 + (1-p)n) \to \infty \quad \text{ as } n \to \infty$$
